...
function handleDownloadExcelTemplate() {
    if (typeof XLSX === 'undefined') {
        showToast("엑셀 라이브러리(SheetJS)가 로드되지 않았습니다.", true);
        return;
    }

    const headerRow = ['아이콘', '관광지명', '설명', '이미지URL', '비용', '기타메모'];
    const sampleData = [
        ['🏛️', '왓 아룬', '방콕의 대표적인 사원, 새벽사원으로 유명', 'https://example.com/watarun.jpg', '฿100', '반바지/민소매 입장 불가'],
        ['🍽️', '소이 카우보이', '방콕 대표 야시장 먹거리', '', '฿200~500', '저녁 시간 방문 추천'],
        ['🏞️', '에라완폭포', '7단 폭포, 수영 가능', '', '฿300', '수건/수영복 준비']
    ];
...
async function handleExcelUpload(event) {
    ...
    if (!db) {
        showToast("Firestore가 초기화되지 않았습니다.", true);
        excelUploadInput.value = '';
        return;
    }
    ...
    try {
        ...
        if (jsonData.length === 0) {
            showToast("엑셀 파일에 데이터가 없습니다.", true);
            excelUploadInput.value = '';
            return;
        }
        ...
        if (validAttractions.length === 0) {
            showToast("업로드할 유효한 데이터가 없습니다. '관광지명' 열을 확인해주세요.", true);
            excelUploadInput.value = '';
            return;
        }
        ...
        showToast(`✅ ${successCount}개의 관광지가 성공적으로 업로드되었습니다.`);
        loadAttractions(); // 목록 새로고침

    } catch (error) {
        console.error("Excel upload error:", error);
        showToast("엑셀 업로드 중 오류가 발생했습니다: " + error.message, true);
    } finally {
        excelUploadInput.value = '';
    }
}
